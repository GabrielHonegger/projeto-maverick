...
export async function proxy(request: NextRequest) {
  try {
    ...
    return new NextResponse(
      `<html>
        <head>
          <title>Erro de Execução no Proxy - Agus Moto Conceito</title>
          <meta charset="utf-8" />
          <style>
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: #09090b; color: #f4f4f5; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
            .card { background: #18181b; border: 1px border #27272a; padding: 2.5rem; border-radius: 1.5rem; max-width: 600px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); width: 90%; }
            h1 { color: #ef4444; font-size: 1.5rem; margin-top: 0; font-weight: 750; }
            p { font-size: 0.9rem; line-height: 1.5; color: #a1a1aa; }
            pre { background: #27272a; padding: 1rem; border-radius: 0.5rem; color: #f4f4f5; font-family: monospace; font-size: 0.8rem; overflow-x: auto; white-space: pre-wrap; word-break: break-all; }
          </style>
        </head>
        <body>
          <div class="card">
            <h1>Erro de Execução no Proxy</h1>
            <p>Ocorreu um erro no arquivo <code>proxy.ts</code> durante a autenticação:</p>
